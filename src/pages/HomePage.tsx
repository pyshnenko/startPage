...
export default function Menu({setUser, user, api, darkMode, width}: InpData) {
    let grow = getGrowIn();
                            ...
                            {textFields.map((dat)=>{ return (
                                <Box key={dat.index} sx={{width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between'}}>
                                    <Box sx={styleText.central}>
                                        {(!edit.activate||dat.index==='login'||dat.index==='telegram')&&<Typography sx={styleText.name} variant={width>500?"h6":"h5"} gutterBottom>{dat.text}</Typography>}
                                        {edit.activate&&dat.index!=='login'&&dat.index!=='telegram' ? 
                                            ...
                                            (dat.index!=='telegram'||(dat.index==='telegram'&&(user[dat.index]!==''||(user.telegramID))))&&<Typography sx={styleText.text} variant={width>500?"h6":"h5"} gutterBottom>
